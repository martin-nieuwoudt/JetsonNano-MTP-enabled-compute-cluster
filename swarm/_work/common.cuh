...
static __device__ int ggml_cuda_get_physical_warp_size_DISABLED() {
...
#define ggml_cuda_get_max_cpy_bytes() 8
#ifndef CUDA_R_16BF
#define CUDA_R_16BF 2 // stub for CUDA 10.2
#endif
...
// C++14 compatible is_any
template <typename T, typename... Ts> struct is_any_impl;
template <typename T> struct is_any_impl<T> { static const bool value = false; };
template <typename T, typename U, typename... Ts>
struct is_any_impl<T, U, Ts...> { static const bool value = std::is_same<T, U>::value || is_any_impl<T, Ts...>::value; };
template <typename T, typename... Ts>
struct is_any { static const bool value = is_any_impl<T, Ts...>::value; };
...
template <typename T> struct block_reduce_policy<block_reduce_method::SUM, T> {
    static __device__ T reduce(T val) {
        if (is_any<T, float, float2, half2, int>::value) {
            ...
            (void)0; // unsupported type
        }
    }

    static __device__ T sentinel() { T z; memset(&z, 0, sizeof(T)); return z; }
};
...
template <typename T> struct block_reduce_policy<block_reduce_method::MAX, T> {
    ...
    static __device__ T sentinel() { T z; memset(&z, 0xFF, sizeof(T)); return z; } // -INFINITY for float, half2
};
...
static __device__ __forceinline__ void ggml_cuda_memcpy_1(void * __restrict__ dst, const void * __restrict__ src) {
    if (alignment != 0) {
        // static_assert removed for CUDA 10.2 compat
    }
...
#pragma unroll
    for (int i = 0; i < nbytes/nb_per_cpy; ++i) {
        if (nb_per_cpy == 1) {
            ...
            // static_assert removed for CUDA 10.2
        }
    }
}
...
struct ggml_cuda_concurrent_event {
    ...
    bool is_valid() const {
        ...
        for (const auto & _p : stream_mapping) { const auto & tensor = _p.first; const auto & stream = _p.second;
...
struct ggml_backend_cuda_context {
    ...
    ggml_cuda_graph * cuda_graph(const void * first_node_ptr) {
        ...
        if (it == cuda_graphs.end()) {
            cuda_graphs[first_node_ptr] = std::unique_ptr<ggml_cuda_graph>(new ggml_cuda_graph());
    ...
    bool any_cuda_graph_enabled() const {
        for (const auto & kv : cuda_graphs) { const auto & graph = kv.second;
