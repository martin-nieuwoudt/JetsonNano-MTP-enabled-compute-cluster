...
keys_want = ('general.', 'llama.', 'qwen.', 'qwen2', 'ggml.', 'model.', 'context',
             'architecture', 'chat', 'block_count', 'embedding', 'rope', 'attention',
             'ssm', 'mtp', 'expert', 'vocab', 'split', 'n_ctx', 'draft', 'head_count',
             'context_length', 'layer', 'tokenizer.ggml.model', 'tokenizer.ggml.pre')
with open(path, 'rb') as f:
    ...
    for i in range(kc):
        ...
        v=rv(f,vt)
        if any(k.startswith(w) for w in keys_want) and 'tokens' not in k and 'chat_template' not in k:
            print(f"{k} = {v}")
