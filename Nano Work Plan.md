...
| 15 | Power Management | Master PC + Dashboard | Distinct "OS Shutdown" (graceful SSH halt) vs Sonoff/Alexa 5V power; watchdog stand-down flag | ✅ (design + flag + MCP tools + dashboard buttons done; NFS wait-and-retry in firstboot still TODO) |
...
> - **Fleet temporarily reverted to stable CUDA build** (`b56f079e2`, `rpc-server`, `-m 3600`, CUDA ON) on all 11 nodes. The MTP build (`b9886`, CPU-only, `GGML_CUDA=OFF`) was the root cause of the RPC crash (`0xC0000409`) — without CUDA the servers reported ~14 MB free memory and couldn't allocate weight buffers. The stable build worked, ran Maxwell GPU compute, and generated Phase 1 output across three models (Phi-3-mini 13.7 t/s, Codestral-22B 2.3 t/s, DeepSeek-R1-32B 2.0 t/s). **This revert was superseded on 2026-07-20** — the MTP CUDA port (Phases A–F) was completed and the fleet now runs the MTP `ggml-rpc-server` build (`b9886`/`20a04b2`, CUDA ON) serving Qwythos-9B. See `MTP CUDA Enablement Work Plan.md` for the full port record.
...
> - **Fleet invariant (at time of writing):** the live 11-node fleet was the stable CUDA build (`b56f079e2`, `rpc-server`, `-m 3600`, CUDA ON) from the 2026-07-15 revert. The MTP build was a **separate** tree on node0 (`/home/jetson/llama.cpp-mtp`, planned port 50053). **This was superseded on 2026-07-20** — the MTP build was deployed fleet-wide (see the 2026-07-20 final note above and `MTP CUDA Enablement Work Plan.md`).
...
> **Status notes (2026-07-20 — FINAL, fleet live):** the cluster is **fully operational** on the MTP stack.
> - **Fleet:** all 11 nodes run the MTP `ggml-rpc-server` build (`b9886`/`20a04b2`, CUDA ON) as a boot-persistent systemd service `llama-rpc-mtp.service` on port 50052. The old `b56f079e2` stable `rpc-server` is retired for Qwythos (MTP models require the MTP build end-to-end — coordinator + workers must both be MTP).
> - **Coordinator (PC):** `C:\llama.cpp-mtp\build\bin\llama-server.exe` is launched as a **persistent daemon** on Load; all chats go through fast HTTP `/completion` calls (no per-prompt `llama-cli` reload). The `llama-cli.exe` in the same dir is the MTP RPC client used for one-shot CLI runs.
> - **Model:** `Qwythos-9B-Claude-Mythos-5-1M-MTP-Q8_0.gguf` (qwen35 arch, MTP draft heads) loads and generates across all 11 nodes at ~6.0 tok/s via the dashboard at http://localhost:9090.
> - **Sampling:** deterministic profile `temp 0.1 / min_p 0.05 / top_p 0.9 / repeat_penalty 1.1` (from `code/mcp/cluster_settings.json`) is enforced end-to-end and unchanged. `ctx_size` 4096, `max_tokens` 4096.
> - **Dashboard:** `code/cluster_telemetry.py web` (port 9090) — Load/Eject/Reset + single + ensemble chat, model-status, thermal/RAM/UMA cards, SSD panel. `code/cluster_server.py` provides the HTTP `/completion` + ensemble fan-out.
> - **MCP server:** 27 tools (added `cluster.method.*` — `method_list`, `method_run`, `method_push` — for the on-node simulation-method library in `code/methods/`).
