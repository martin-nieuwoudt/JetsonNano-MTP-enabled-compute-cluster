<#
.SYNOPSIS
    Wait for a Jetson worker to come up after its card is plugged in / powered
    on, then verify it joins the cluster as an RPC worker.

.DESCRIPTION
    The per-worker card (Jetson_Worker_nanoNN.img) is already flashed and
    "ready to plug". This script does NOT launch Rufus. It polls for the board
    to appear on the network, then verifies:
        - hostname == nanoNN
...
.PARAMETER WaitSeconds
    How long to wait for the worker to respond to ping. Default 600.

.PARAMETER PollSeconds
    Poll interval. Default 5.

.EXAMPLE
    .\verify_worker.ps1 -WorkerNumber 6
#>
[CmdletBinding()]
param(
...
Write-Host "=== verify_worker : waiting for $name ($ip) to come up (max ${WaitSeconds}s) ==="
