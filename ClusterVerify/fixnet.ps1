$log = "C:\ClusterVerify\fixnet_log.txt"
"$(Get-Date) START offline network fix" | Out-File -Append $log
try {
    ...
    $r = wsl -d Ubuntu -u root -e bash /mnt/c/ClusterVerify/fixnet.sh 2>&1
