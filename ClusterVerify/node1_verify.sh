...
echo "=== node1: are the 5 files now valid XML? ==="
ssh $OPTS $N1 'bad=0; for f in org.freedesktop.PolicyKit1.conf com.canonical.UbuntuAdvantage.conf io.netplan.Netplan.conf org.freedesktop.fwupd.conf org.freedesktop.bolt.conf; do
  if [ -f /usr/share/dbus-1/system.d/$f ]; then
    ...
    echo "$f MISSING"; bad=$((bad+1))
  fi
done; [ "$bad" -eq 0 ] && echo "ALL 5 RESTORED VALID XML"' 2>&1

echo "=== node1: dbus-daemon running? ==="
ssh $OPTS $N1 'pgrep -a dbus-daemon | head -2' 2>&1

echo "=== node1: polkit active? (no NM restart) ==="
ssh $OPTS $N1 'systemctl is-active polkit 2>&1; systemctl is-active NetworkManager 2>&1' 2>&1

echo "=== node1: systemctl reachable over bus? ==="
ssh $OPTS $N1 'systemctl list-units --type=service --no-legend 2>&1 | head -3' 2>&1
echo DONE
